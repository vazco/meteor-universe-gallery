...
UniGallery.addCollection('GalleryFiles', {
    onInit: function (collection) {
        collection.Schema = new SimpleSchema({
            fileId: {
                type: String
...
UniGallery.getGalleryFiles = function (gallery_id){
    ...
    var files_private = [];
    UniGallery.GalleryFiles.find({_id: {$in: gallery.files}}).forEach(function (doc){
...
UniGallery.removeFileFromGallery = function (gallery_id, file_cfs_id) {
    var file = UniGallery.GalleryFiles.findOne({fileId: file_cfs_id});
    var file_id, gallery;
    ...
    UniGallery.GalleryFiles.remove(file_id);
    return true;
};